...
export async function getProductInventory(productId) {
  return await IMSInventory.find({ productId })
    .populate('warehouseId')
    .lean();
}

// Add stock (IN operation)
export async function addStock(productId, warehouseId, size, quantity, userId, reason = '', referenceNumber = '') {
...
export async function removeStock(productId, warehouseId, size, quantity, userId, reason = '', referenceNumber = '') {
  const session = await mongoose.startSession();
  session.startTransaction();
  
  try {
    // Check current stock
    const inventory = await IMSInventory.findOne({ productId, warehouseId, size }).session(session);
    
    if (!inventory || inventory.quantity < quantity) {
      throw new Error(`Insufficient stock. Available: ${inventory?.quantity || 0}, Requested: ${quantity}`);
    }
...
export async function transferStock(productId, fromWarehouseId, toWarehouseId, size, quantity, userId, reason = '', referenceNumber = '') {
  const session = await mongoose.startSession();
  session.startTransaction();
  
  try {
    // Check source warehouse stock
    const fromInventory = await IMSInventory.findOne({ 
      productId, 
    ...
    if (!fromInventory || fromInventory.quantity < quantity) {
      throw new Error(`Insufficient stock in source warehouse. Available: ${fromInventory?.quantity || 0}, Requested: ${quantity}`);
    }
    
    // Deduct from source
    fromInventory.quantity -= quantity;
    fromInventory.lastUpdated = new Date();
    fromInventory.updatedBy = userId;
    ...
    await session.commitTransaction();
    return { success: true, fromInventory };
  } catch (error) {
    await session.abortTransaction();
    throw error;
  } finally {
...
export async function recordSale(productId, warehouseId, size, quantity, userId, orderNumber) {
  return await removeStock(productId, warehouseId, size, quantity, userId, 'Sale', orderNumber);
}

// Record return (adds stock back)
export async function recordReturn(productId, warehouseId, size, quantity, userId, orderNumber) {
  const session = await mongoose.startSession();
  session.startTransaction();
